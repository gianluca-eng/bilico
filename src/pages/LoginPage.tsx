import { Navigate } from 'react-router-dom';
import { useState } from 'react';
import { useAuth } from '../hooks/useAuth';
import { useStore } from '../lib/store';
import { BalanceScale, Wordmark, EuroPill } from '../components/BalanceScale';
import { BigButton } from '../components/Ui';
import {
  H, B, SERIF,
  INK, CREAM, CORAL, MINT,
} from '../components/tokens';

function GoogleG() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" aria-hidden>
      <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
      <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
      <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
      <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
    </svg>
  );
}

export default function LoginPage() {
  const { user, signInWithGoogle } = useAuth();
  const { profile, authLoading } = useStore();
  const [signing, setSigning] = useState(false);
  const [error, setError] = useState('');

  // Se l'utente è loggato, aspetta che il profilo sia caricato
  // prima di decidere dove mandarlo, altrimenti si può ritrovare
  // in onboarding anche se ha già completato il setup.
  if (user && !authLoading) {
    return <Navigate to={profile?.onboardingComplete ? '/dashboard' : '/onboarding'} replace />;
  }

  const handleGoogle = async () => {
    setError('');
    setSigning(true);
    try {
      await signInWithGoogle();
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Accesso non riuscito');
    } finally {
      setSigning(false);
    }
  };

  return (
    <div style={{
      minHeight: '100svh',
      background: CREAM,
      padding: 'calc(40px + env(safe-area-inset-top, 0px)) 24px calc(28px + env(safe-area-inset-bottom, 0px))',
      boxSizing: 'border-box',
      display: 'flex',
      flexDirection: 'column',
      maxWidth: 420,
      margin: '0 auto',
      width: '100%',
    }}>
      {/* Brand + € pill */}
      <div style={{
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'space-between',
        marginBottom: 6,
      }}>
        <Wordmark />
        <EuroPill />
      </div>

      {/* Bilancia animata */}
      <BalanceScale spent={880} free={1320} />

      {/* Headline editoriale */}
      <div style={{ position: 'relative', margin: '6px 0 6px' }}>
        <h1 style={{
          ...H,
          fontWeight: 800,
          fontSize: 36,
          color: INK,
          lineHeight: 1.02,
          letterSpacing: '-1.2px',
          margin: 0,
        }}>
          Tieni i soldi<br />
          <span style={{
            ...SERIF,
            fontStyle: 'italic',
            fontWeight: 400,
            color: CORAL,
            fontSize: 40,
          }}>in bilico.</span>
        </h1>
        <div aria-hidden style={{
          position: 'absolute',
          right: 6,
          top: 42,
          width: 24,
          height: 24,
          borderRadius: '50%',
          background: MINT,
          border: `2px solid ${INK}`,
        }} />
      </div>

      <p style={{
        ...B,
        fontSize: 14,
        color: INK,
        opacity: 0.72,
        lineHeight: 1.45,
        margin: '4px 0 18px',
        maxWidth: 300,
      }}>
        Senza fogli Excel, senza sensi di colpa. Solo tu e i tuoi soldi che si parlano.
      </p>

      {/* CTA */}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        <BigButton
          variant="ink"
          onClick={handleGoogle}
          disabled={signing}
          icon={
            <span style={{
              width: 22,
              height: 22,
              borderRadius: '50%',
              background: CREAM,
              display: 'inline-flex',
              alignItems: 'center',
              justifyContent: 'center',
            }}>
              <GoogleG />
            </span>
          }
        >
          {signing ? 'Accesso in corso…' : 'Entra con Google'}
        </BigButton>
      </div>

      {error && (
        <p style={{
          ...B,
          fontSize: 12,
          color: CORAL,
          textAlign: 'center',
          marginTop: 10,
        }}>
          {error}
        </p>
      )}

      <p style={{
        ...B,
        fontSize: 11.5,
        color: INK,
        opacity: 0.55,
        textAlign: 'center',
        lineHeight: 1.5,
        margin: '14px 0 0',
      }}>
        Niente pubblicità. Niente popup sui risparmi.<br />
        Solo <span style={{ ...SERIF, fontStyle: 'italic', fontSize: 14 }}>chiarezza</span>.
      </p>
    </div>
  );
}
